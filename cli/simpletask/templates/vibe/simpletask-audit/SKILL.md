...
4. If a `gilfoyle` review agent or equivalent delegation mechanism is available in the environment, you may use it for the initial roast. Otherwise perform the audit inline and continue without asking.
...
2. Determine diff base from `summary.latest_audit_head_sha`, falling back to `summary.latest_audit_base_sha`, then `main` or `master` when needed.
...
5. Run the unrestricted review:
   - Use an available `gilfoyle`-style review agent if the environment supports it.
   - Otherwise perform the same audit inline against the diff.
...
2. Present `RECOMMENDED TO FIX` and `OPTIONAL / COSMETIC` buckets and wait for user approval.
3. After approval:
    - `simpletask_iteration(action="add", label="audit fixes")`
    ...
    - Create fix tasks only for approved findings that are truly new and not already tracked, using `simpletask_task(action="add", ...)` or `action="batch"`.
4. Persist audit history with `simpletask_audit(action="add_run", iteration=..., base_sha=DIFF_BASE, head_sha=HEAD, findings=[...])` for all reviewed findings, including false positives and uncertain items.
5. Validate with `simpletask_get(validate=True, full=True)`.
