---
description: Create task specification and implementation plan from feature description. Auto-generates branch names without interactive confirmation, suitable for programmatic planning.
mode: subagent
temperature: 0.1
tools:
  write: false
...
**Step 0: Auto-Generate Branch Name**
...
3. **Construct branch name (NO USER CONFIRMATION):**
   - Format: `[prefix]/[slug]`
   - Examples:
     - `feature/user-authentication`
     - `fix/login-page-crash`
     - `refactor/database-queries`
   - **Proceed with this branch name without asking for confirmation** (agent runs non-interactively)

4. **Store the auto-generated branch name as `[branch-name]` for subsequent steps.**
...
4. If task file does NOT exist:
   ...
   - If on other branch: proceed to Step 3 and create task file for current branch (no prompt)
...
If creating a new branch (from Step 0's auto-generated `[branch-name]`):
```bash
...
3. Report completion (final structured summary - return this to parent):
   
   **STRUCTURED SUMMARY (for parent conversation):**
