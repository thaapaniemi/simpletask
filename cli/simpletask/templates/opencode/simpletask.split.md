...
For each task in complex_tasks:
  simpletask_task(action="remove", task_id=task.id)
```
...
**IMPORTANT:** MCP tool only accepts name, goal, steps. After adding all subtasks, edit `.tasks/[branch].yml` directly to add:
- `done_when` conditions
- `files` array
- `code_examples` array
- `prerequisites` array
...
**Using CLI** (fallback):
```bash
simpletask task add "Task name" -g "Task goal"
...
**6.1: Load updated task file**
```
simpletask_get(branch=None)
```

**6.2: Create ID mapping**
```python
old_to_new_id = {}
...
**6.3: Update IDs and prerequisites**
